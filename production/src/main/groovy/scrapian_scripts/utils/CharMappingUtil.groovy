...
class CharMappingUtil {
	public String latinize(String toBeLatinized){
		if(toBeLatinized == null){
			return null;
		}
		StringBuilder retVal = new StringBuilder();
		for(char c : toBeLatinized.toCharArray()){
			retVal.append(removeAccents(c));
		}
		return retVal.toString();
	}

	public String removeAccents(char c) {
		char output;
		Character output2 = null;
		if (c < '\u0100') {
			//bypass if was already in Latin1
			output = c;
		} else {
			switch (c) {
		...
		return (output2 == null ? new String("" + output) : new String("" + output + output2));
	}
}
