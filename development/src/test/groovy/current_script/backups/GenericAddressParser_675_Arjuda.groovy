package current_script.backups
...
class GenericAddressParser_675_Arjuda {
  private ScrapianContext context
  private String xlsLocation = "../assets/config/standard_addressees/"
  ...
  protected cityIgnoreSuffixes = /(?i)\b(?>Roads?|Ave|St)\b/

    GenericAddressParser_675_Arjuda(ScrapianContext context, xlsLocation = null) {
    this.context = context
    if (xlsLocation) {
      this.xlsLocation = xlsLocation.replaceAll(/(?<!\/)$/, "/")
    }
    init()
  }
  ...
  private enum XLS_EXTRA_TOKENS {
    EXPECTED_STATES("esl"), OTHER_MAPPED_COUNTRIES("ocl");

    String value

    XLS_EXTRA_TOKENS(String val) {
      value = val;
    }
  }
  ...
  Map<TOKENS, String> parseAddress(String addr, options = [:]) {
    ...
    if (result[TOKENS.COUNTRY]) {
      return parse2CharUsaProvinceAddress(result, options["delimiter"]) ? result : findCountryParts(result)
    } else {
  ...
  Map<TOKENS, String> parseAddress(Map options) {
    ...
    def addrMap = parseAddress(options["text"], options)
        ...
        def cityCollector = {
          if (!addrMap[TOKENS.CITY]) {
            ...
            addrMap[TOKENS.ADDR_STR] = addrMap[TOKENS.ADDR_STR].toString().replaceAll(/((?:^|.*?$delim+))([^$delim]*(?=$cityTokens)\w+)$delim+(.*+)/, { a, b, c, d ->
             //if city contains both village and district(EX: Siyachoy village, Panjwai District)
              if(!(c=~/(?i)village/) && !(d =~ /(?i)(?:district|city)/)) {
        ...
        def stateCollector = {
          if (!addrMap[TOKENS.STATE]) {
            if (addrMap[TOKENS.COUNTRY] != "UNITED STATES") {
              ...
              parse2CharUsaProvinceAddress(addrMap, delim, false)
            }
          }
        }
        stateCollector.call()
  ...
  /**
   * Special parser for 2 char stated USA addressees
   * ie.: 705 Hickory, PO Box 64, Stanton, NE 68779, USA
   *
   * return true (two char state is found) / false
   * */
  def parse2CharUsaProvinceAddress(addrMap, delim, isFullCapture = true) {
    def isParsed = false

    if (addrMap[TOKENS.COUNTRY] == "UNITED STATES") {
      if (isFullCapture && !addrMap[TOKENS.ZIP]) {
        findPostalCodes(addrMap)
      }
      def pattern = /\s*$delim+[^\w$delim]*+\b((?-i:[A-Z]{2}))\b\W*$/
      def isCaptureCity = isFullCapture && !addrMap[TOKENS.CITY]
      if (isCaptureCity) {
        pattern = /(?s)(.*)$delim+[^\w$delim]*+([^$delim]++)$pattern/
      ...
      addrMap[TOKENS.ADDR_STR] = addrMap[TOKENS.ADDR_STR].toString().replaceAll(pattern, {
        def stateTmp = it[-1].toUpperCase()
        def state = new StateMappingUtil().normalizeState(stateTmp)
        if (state != stateTmp) {
          //2words state to Full state found
          addrMap[TOKENS.STATE] = state
          if (isCaptureCity) {
            addrMap[TOKENS.CITY] = it[2].trim()
          }
          isParsed = true
          return it[1] //return group 1

        } else {
          return it[0] //return full match
        }
      })
    }
    ...
    def isValidMatch = {match ->
      if (match[0]) {
        if (addrStr.contains(delimiter)) {
          ...
          log("No matching delimiter found! Either provide a valid delimiter in the input address or ignore this warning. Addr: " + addrStr)
          return true
        }
      }
  ...
  private log(msg, isActive = false) {
...
//TODO: multiple mixed address splitting method based on ending country|state phrase I.E. "Tower 42, 25 Old Broad Street, London, UK Mainzer Landstrae 16-24, 60325 Frankfurt, Germany"
//TODO: find a way to resolve same city|state(ie. maryland) name for multiple different country in the case of force address parsing