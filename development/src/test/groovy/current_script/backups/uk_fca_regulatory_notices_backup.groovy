...
Uk_fca script = new Uk_fca(context)
...
//    tasker.execute({handleDetailsPage(url, it)}, taskRetryCount)
  ...
  def handleDetailsPage(def pageUrl, def searchCatagory) {
    ...
    while (loopVar || loopVar2) {
      ...
      def html = context.invoke([url: nextPageUrl, tidy: true])
//      def tempBool = tasker.execute({detailsPageParsing(html, searchCatagory)}, taskRetryCount)
      def tempBool = detailsPageParsing(html, searchCatagory)
      if (tempBool) {
        loopVar = true
      } else {
        if (loopVar) {
 ...
 */
  ...
  def detailsPageParsing(html, searchCatagory) {
    ...
    def endMatch = html =~ /0 search results/
    if (endMatch.find()) {
      return false
    }
    ...
    while (rowMatch.find()) {
      ...
      row = row.replaceAll("<strong>", "").replaceAll("</strong>", "")
      // get rid of <strong> tags to allow regex to work
...
//------------------------------Entity creation part---------------//
  ...
  def createEntityCommonCore(entity, attrMap) {
    ...
    if (attrMap[FIELDS_UK_FCA.COUNTRY]) {
      ...
      addr.setProvince(attrMap[FIELDS_UK_FCA.STATE])
  ...
  def getAddress(attrMap) {
    ...
    def pmap = ["1": "-l",
                "2": "1",
                "3": "-layout"] as Map
    def gbPostalCode = /(?is)\b[a-z][a-z]?\d[a-z\d]?(?:[\s-]+\d[^cikmov\W\d]{2})?\b(?=\W*$)/
    // skip these bad addresses
    def badAddresses = "16 The Courtyard, Buntsford Drive, B60 3DJ|" +
