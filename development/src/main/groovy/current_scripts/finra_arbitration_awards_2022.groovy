...
Finra_Arbitration_Awards_2022 script = new Finra_Arbitration_Awards_2022(context)
script.initParsing()


class Finra_Arbitration_Awards_2022 {
    final addressParser
    final entityType
    final def ocrReader
    ...
    def url = 'https://www.finra.org/arbitration-mediation/arbitration-awards-online?aao_radios=all&search=&field_case_id_text=&field_core_official_dt%5Bmin%5D=01/01/2022&field_core_official_dt%5Bmax%5D=12/31/2022&field_document_type_tax%5B4224%5D=4224&field_forum_tax=All&field_special_case_type_tax=All&page='
    def prefixPdfUrl = 'https://www.finra.org'

    Finra_Arbitration_Awards_2022(context) {
    ...
    def sanitizePdfText(def pdfText) {
        pdfText = pdfText.toString().trim()
            .replaceAll(/[\“\”]/, "\"")
            .replaceAll(/[\.]+/, ".")
            .replaceAll(/(?i),\s*(?=L\.L\.P|L\.L\.C|Inc\.?|LLC|LLLP|JR\.?|SR\.?|Rose|a\/Ida|LLP|ET|LTD|FENNER|PIERCE|a\/k\/a|AKA|LP|III|& Smith|Berland|Sailer|IV|II)/, " ")
