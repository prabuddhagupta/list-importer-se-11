...
Finra_Arbitration_Awards_Twenty_TwentyOne script = new Finra_Arbitration_Awards_Twenty_TwentyOne(context)
script.initParsing()


class Finra_Arbitration_Awards_Twenty_TwentyOne {
    final addressParser
    final entityType
    final def ocrReader
    ...
    def url = 'https://www.finra.org/arbitration-mediation/arbitration-awards-online?aao_radios=all&search=&field_case_id_text=&field_core_official_dt%5Bmin%5D=01/01/2021&field_core_official_dt%5Bmax%5D=12/31/2021&field_document_type_tax%5B4224%5D=4224&field_forum_tax=All&field_special_case_type_tax=All&page='

    def urlHeader = 'https://www.finra.org/arbitration-mediation/arbitration-awards-online'
    def prefixPdfUrl = 'https://www.finra.org'
    def testUrl = 'https://www.finra.org/sites/default/files/aao_documents/18-02661.pdf'

    Finra_Arbitration_Awards_Twenty_TwentyOne(context) {
    ...
    def handleRow(def row) {
        ...
        if (pdfDateAddressMatcher.find()) {
            ...
            def text
            //if (!pdfNotContains(pdfUrl)) {
                text = pdfToTextConverter(pdfUrl)
           // }
            def remark = pdfDateAddressMatcher.group(2)
            def address = pdfDateAddressMatcher.group(3)
            def date = pdfDateAddressMatcher.group(4)
    ...
    def pdfNotContains(def entityUrl) {
        ...
        if (entityUrl.toString().contains("21-00917.pdf") || entityUrl.toString().contains("17-00265.pdf") || entityUrl.toString().contains("19-03802.pdf")|| entityUrl.toString().contains("20-03228.pdf")|| entityUrl.toString().contains("20-02707.pdf")|| entityUrl.toString().contains("21-01497.pdf")
            || entityUrl.toString().contains("21-00299.pdf")|| entityUrl.toString().contains("21-01118.pdf")|| entityUrl.toString().contains("20-00221.pdf")|| entityUrl.toString().contains("20-03644.pdf")|| entityUrl.toString().contains("20-03678.pdf")|| entityUrl.toString().contains("20-03020.pdf")
            || entityUrl.toString().contains("20-03486.pdf")|| entityUrl.toString().contains("20-00569.pdf")|| entityUrl.toString().contains("19-03104.pdf")|| entityUrl.toString().contains("21-00348.pdf")) {
    ...
    def sanitizePdfText(def pdfText) {
        pdfText = pdfText.toString().trim()
            .replaceAll(/[\“\”]/, "\"")
            .replaceAll(/[\.]+/, ".")
            .replaceAll(/(?i),\s*(?=L\.L\.P|L\.L\.C|Inc\.?|LLC|LLLP|JR\.?|SR\.?|Rose|a\/Ida|LLP|ET|LTD|FENNER|PIERCE|a\/k\/a|AKA|LP|III|& Smith|Berland|Sailer)/, " ")
    ...
    def sanitizeEntitiesData(def entities) {
        entities = entities.toString().trim()
            ...
            .replaceAll(/(?<=(?:Rosenthal))\s+(?=(?:James Coyle))/, ",")
    ...
    def sanitizePdfTextForAddress(def pdfText) { return pdfText.toString().trim() }
    ...
    def sanitizeDescription(def alias) {
        alias = alias.toString().trim()
            .replaceAll(/\s+/, " ")
            .replaceAll(/[,;]/, " ")
            .replaceAll(/\^/, "")
    ...
    def sanitizeAddress(def address) {
        address = address.toString().trim()
            .replaceAll(/[\'\}\-\:,\|]$/, "").trim()
            .replaceAll(/(?:tilinois|\/inois)/, "Illinois").trim()
            .replaceAll(/(?:, Georgia)/, ", GA").trim()
            .replaceAll(/\s[\|\.i,;\*]$/, "")
            .replaceAll(/\s*$/, ", USA").trim()
            .replaceAll(/^(.*)$/, ', $1')
        return address.trim()
    }
